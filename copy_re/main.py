...
def test_all_models(model_epochs, data, decoder, sess, config):
    ...
    for epoch in model_epochs:
        ...
        data.reset()
        f1, precision, recall = test_model(data, decoder, sess, show_rate=0.1, is_visualize=False)
...
def train_NLL_model(data, epoch_range, decoder, sess):
    ...
    for epoch_i in epoch_range:
        ...
        for batch_i in range(data.batch_number):
            ...
            if batch_i % 100 == 0 :
                logger.info('step {}:\tloss:{}'.format(epoch_i*data.batch_number + batch_i, loss_val))
...
if __name__ == '__main__':
    ...
    elif config.dataset_name == const.DataSet.KG:
        prepare = data_prepare.KGPrepare(config)
    else:
    ...
    data = data_prepare.Data_(data, config.batch_size, config)

    if is_train:
        logger.info('****************************** NLL Train ******************************')
        train_NLL_model(data, epoch_range=list(range(1, config.epoch_number + 1)), decoder=decoder, sess=sess)
    else:
        logger.info(
        ...
        # =============
        #model_epoch = list(range(1, 51, 1))
        model_epoch = range(2, 100, 2)
